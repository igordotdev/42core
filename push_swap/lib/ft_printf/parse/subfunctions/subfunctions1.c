...
{
	if (flags->width > (int)fft_strlen(str, 0))
	{
		if (flags->minus)
		{
			temp = (char *)fft_calloc((size_t)flags->width
					- fft_strlen(str, 0) + 1, 1);
			if (temp == NULL)
				return (NULL);
			temp = ft_memset(temp, ' ',
					(size_t)flags->width - fft_strlen(str, 0));
			str = fft_strjoin(str, temp);
		}
		else
		{
			temp = (char *)fft_calloc((size_t)flags->width
...
void	sub_handle_nint1(t_flags *flags, char *str, char *anum)
{
	...
	i = 0;
	str[i++] = '-';
	if (flags->precision > (int)fft_strlen(anum, 0))
		while (i < flags->precision - (int)fft_strlen(anum, 0) + 1)
			str[i++] = '0';
	ft_memcpy(str + i, anum, fft_strlen(anum, 0));
	i += (int)fft_strlen(anum, 0);
	while (i < flags->width)
		str[i++] = ' ';
}
