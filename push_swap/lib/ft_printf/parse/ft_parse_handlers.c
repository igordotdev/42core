...
char	*ft_handle_char(char chr, t_flags *flags)
{
	...
	str = (char *)fft_calloc((size_t)(flags->width + 2), sizeof(char));
	if (chr == 0)
		flags->is_null = 1;
	if (str == NULL)
		return (NULL);
...
char	*ft_handle_string(char *str, t_flags *flags)
{
	...
	if (str == NULL && flags->dot && flags->precision < 6)
		...
		str = "(null)";
	len = (int)fft_strlen(str, 0);
	if (flags->precision >= 0 && flags->precision < len && flags->dot)
		len = flags->precision;
	new_str = (char *)fft_calloc(\
			(size_t)(flags->width + 2 + len), sizeof(char));
	if (new_str == NULL)
		return (NULL);
...
char	*ft_handle_pointer(void *address, t_flags *flags)
{
	...
	{
		temp = fft_itoa_base((intptr_t)address, HEX);
		if (temp == NULL)
			return (NULL);
		str = fft_strjoin("0x", temp);
		free(temp);
		if (str == NULL)
			return (NULL);
...
char	*ft_handle_nint(unsigned int num, t_flags *flags)
{
	...
	if (flags->width > len)
		...
		str = (char *)fft_calloc((size_t)(len + 2), sizeof(char));
	if (str == NULL)
		return (NULL);
	if (flags->minus)
